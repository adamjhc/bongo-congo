...
public class TextObject extends GuiObject {
  ...
  /**
   * Builds the mesh Quad is made up of a set of tiles each one being a single character Assigns the
   * texture coordinates depending on character Loops through for each character
   *
   * @return Mesh
   * @author Joseph
   */
  private Mesh buildMesh() {
    ...
    for (int i = 0; i < numChars; i++) {
      ...
      positions.add(ZPOS); // z
      textCoords.add(
          (float) (charInfo.getStartX() + charInfo.getWidth()) / (float) fontTexture.getWidth());
      textCoords.add(1.0f);
  ...
  /**
   * Set the text Save old colour before deleting and rebuilding mesh
   *
