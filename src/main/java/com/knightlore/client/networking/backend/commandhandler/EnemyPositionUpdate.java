...
public class EnemyPositionUpdate implements GenericHandler {
  ...
  public void run(Client client, Sendable sendable) {
    ...
    Gson gson = new Gson();
    EnemyLocationUpdate enemyLocationUpdate =
        gson.fromJson(sendable.getData(), EnemyLocationUpdate.class);

    for (Map.Entry<Integer, EnemyLocationUpdateObject> entity :
        enemyLocationUpdate.updates.entrySet()) {
      GameConnection.gameModel
          .getCurrentLevel()
          .getEnemies()
          .get(entity.getKey())
          .setPosition(entity.getValue().position);
      GameConnection.gameModel
          .getCurrentLevel()
          .getEnemies()
          .get(entity.getKey())
          .setDirection(entity.getValue().direction);
      GameConnection.gameModel
          .getCurrentLevel()
          .getEnemies()
          .get(entity.getKey())
          .setCurrentState(entity.getValue().state);
    }
  }
}
