...
public class LevelComplete extends Command {
  ...
  public void run(ClientHandler handler, Sendable sendable) {
    ...
    Gson gson = new Gson();
    com.knightlore.networking.game.LevelComplete lc =
        gson.fromJson(sendable.getData(), com.knightlore.networking.game.LevelComplete.class);
    ...
    if (allFinished) {
      ...
      //      chunk.add(new PositionUpdate(player.getPosition(), handler.sessionKey.get(),
      // player.getDirection(), player.getPlayerState(), player.getScore()));
