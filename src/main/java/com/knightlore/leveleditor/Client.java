...
public class Client extends Thread {
	...
	private void init() throws Exception {
		window = new Window("Bongo Congo: Level Editor", 1280, 720);
		mouseInput = new MouseInput();
		timer = new Timer();
		audio = new AudioHandler();
		levelReader = new LevelReader("src/main/resources/custom maps/customMap");
		...
		if (model == null) {
			gameModel = new Game("");
			try {
				gameModel.createNewLevel(levelReader.getLevel());
			} catch (Exception e) {
				throw e;
			}
