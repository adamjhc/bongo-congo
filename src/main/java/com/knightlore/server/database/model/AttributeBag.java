...
public class AttributeBag {
    ...
    public ArrayList<String> getDirty(){
        ...
        for(Map.Entry<String, Attribute> current: this.attributes.entrySet()){
            if(current.getValue().isDirty)
                dirty.add(current.getKey());
        }
    ...
    /**
     * Update attributes from bag based on result
     * NB resultset must include all keys
     * @param result
     * @throws SQLException
     */
     ...
     * @throws SQLException
     */
    public PreparedStatement setStatementPrimaryKey(PreparedStatement statement, Object primaryValue) throws SQLException{
    ...
    public PreparedStatement setStatementVariables(PreparedStatement stmt, ArrayList<String> keys) throws SQLException{
        int i = 1;
    ...
    private PreparedStatement setExplicitStatementValues(int i, PreparedStatement stmt,  int type, Object value) throws SQLException{
        switch(type){
            case Types.INTEGER:
    ...
    public PreparedStatement setUpdateStatemntVariables(PreparedStatement stmt, Iterable<String> keys) throws SQLException{
        int i = 1;
        for(String key: keys){
            Attribute current = this.attributes.get(key);
